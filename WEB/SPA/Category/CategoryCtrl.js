app.controller("CategoryCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/Category/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "Description = '" + $scope.entity.Description + "'";
        if ($scope.entity.Id > 0)
            where += " AND Id <> " + $scope.entity.Id;

        $http({
            url: '/Category/GetDynamic?where=' + where + '&orderBy=Description',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
                alertify.log($scope.entity.Description + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
