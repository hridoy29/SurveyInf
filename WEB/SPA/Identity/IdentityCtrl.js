app.controller("IdentityCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/Identity/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "Designation = '" + $scope.entity.Designation + "'";
        if ($scope.entity.Id > 0)
            where += " AND Id <> " + $scope.entity.Id;

        $http({
            url: '/Identity/GetDynamic?where=' + where + '&orderBy=Designation',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
                alertify.log($scope.entity.BBDName + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
