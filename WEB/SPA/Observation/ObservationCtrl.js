app.controller("ObservationCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function clear() {
        $scope.entity = { Id: 0, IsActive: true, IsPositiveComments: true };
        $("#txtFocus").focus();
    };
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/Observation/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        ...
        $http({
            url: '/Observation/GetDynamic?where=' + where + '&orderBy=Name',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
