app.controller("BBDCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/BBD/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "BBDName = '" + $scope.entity.BBDName + "'";
        if ($scope.entity.Id > 0)
            where += " AND Id <> " + $scope.entity.Id;

        $http({
            url: '/BBD/GetDynamic?where=' + where + '&orderBy=BBDName',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
