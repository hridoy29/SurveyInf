app.controller("ItemGroupCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/ItemGroup/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "GroupName = '" + $scope.entity.GroupName + "'";
        if ($scope.entity.Id > 0)
            where += " AND Id <> " + $scope.entity.Id;

        $http({
            url: '/ItemGroup/GetDynamic?where=' + where + '&orderBy=GroupName',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
                alertify.log($scope.entity.GroupName + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
