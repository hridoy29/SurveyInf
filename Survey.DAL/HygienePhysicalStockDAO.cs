...
namespace SurveyDAL
{
    public class HygienePhysicalStockDAO : IDisposable
    {
        private static volatile HygienePhysicalStockDAO instance;
        private static readonly object lockObj = new object();
        public static HygienePhysicalStockDAO GetInstance()
        {
            if (instance == null)
            {
                instance = new HygienePhysicalStockDAO();
            }
            return instance;
        }
        public static HygienePhysicalStockDAO GetInstanceThreadSafe
        {
            get
            {
                if (instance == null)
        ...
        public HygienePhysicalStockDAO()
        {
            //dbExecutor = DBExecutor.GetInstanceThreadSafe;
            dbExecutor = new DBExecutor();
        }

        public List<IssueWisePhysicalStocksReport> Get(DateTime toDate,int distributorId)
        {
            try
            {
                List<IssueWisePhysicalStocksReport> QuestionnaireDetailsReport_getLst = new List<IssueWisePhysicalStocksReport>();
                List<HygeinePhysicalStocksReport> QuestionnairePhysicalStocksReport_getLst = new List<HygeinePhysicalStocksReport>();
                ...
                QuestionnairePhysicalStocksReport_getLst = dbExecutor.FetchData<HygeinePhysicalStocksReport>(CommandType.StoredProcedure, "wsp_HygienePhysicalStockReport_Get", colparameters);

                List < List <HygeinePhysicalStocksReport>> _List = new List<List<HygeinePhysicalStocksReport>>();
                _List  = QuestionnairePhysicalStocksReport_getLst
                                            .GroupBy(u => new { u.DbCode, u.Issue })
                                            .Select(grp => grp.ToList())
                                            .ToList();
            

                foreach (List<HygeinePhysicalStocksReport> item in _List)
                {
                    ...
                    expandable.Address = item[0].Address;
                    expandable.ContactPersonName = item[0].ContactPersonName;
                    expandable.IssueWisePhysicalStockList = item.OrderByDescending(x=>x.Difference).ToList();
